...
class _AttendanceScreenState extends State<AttendanceScreen> {
  ...
  Future<void> printData() async {
    try {
      ...
      final String locStr = "locationdata : ${locBox.toMap()}";

      // Print in safe chunks of 800 characters to prevent IDE debug console truncation
      final int chunkSize = 800;
      int locStart = 0;
      while (locStart < locStr.length) {
        int locEnd = locStart + chunkSize;
        if (locEnd > locStr.length) locEnd = locStr.length;
        print(locStr.substring(locStart, locEnd));
        locStart = locEnd;
      }

      final clockBox = await Hive.openBox('clokINOutData');
      final String clockStr = "clock in out : ${clockBox.toMap()}";

      int clockStart = 0;
      while (clockStart < clockStr.length) {
        int clockEnd = clockStart + chunkSize;
        if (clockEnd > clockStr.length) clockEnd = clockStr.length;
        print(clockStr.substring(clockStart, clockEnd));
        clockStart = clockEnd;
      }
    } catch (e) {
      print("Error printing Hive data in printData: $e");
    }
  }
